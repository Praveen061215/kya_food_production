<?php
/**
 * KYA Food Production - Add Processing Log
 * Form to add new processing logs
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $operator_id = !empty($_POST['operator_id']) ? intval($_POST['operator_id']) : $userInfo['id'];
    ...
    // Calculate yield percentage if output quantity is provided
    ...
    if (empty($errors)) {
        try {
            ...
            if ($result) {
                logActivity('processing_log_added', "Processing log added: Batch {$batch_id}", $userInfo['id']);
                $success = true;
                header("Location: logs.php?section={$section}&success=added");
                exit();
            } else {
                $errors[] = 'Failed to add processing log';
            }

        } catch (Exception $e) {
            error_log('Add processing log error: ' . $e->getMessage());
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 mb-0">
            <i class="fas fa-plus me-2"></i>Add Processing Log
        </h2>
        <div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" name="batch_id" id="batch_id" class="form-control" 
                                   value="<?php echo htmlspecialchars($_POST['batch_id'] ?? ''); ?>" 
                                   placeholder="e.g., BATCH-2024-001" required>
                            <button type="button" class="btn btn-outline-secondary" onclick="generateBatchId()" title="Generate Batch ID">
                        ...
                        <select name="operator_id" id="operator_id" class="form-select">
                            ...
                            <?php foreach ($users as $user): ?>
                                <option value="<?php echo $user['id']; ?>" 
                                        <?php echo (isset($_POST['operator_id']) ? $_POST['operator_id'] : $userInfo['id']) == $user['id'] ? 'selected' : ''; ?>>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="quality_check" id="quality_check" class="form-select">
                            <option value="pending" <?php echo (isset($_POST['quality_check']) && $_POST['quality_check'] == 'pending') ? 'selected' : ''; ?>>Pending</option>
                            <option value="pass" <?php echo (isset($_POST['quality_check']) && $_POST['quality_check'] == 'pass') ? 'selected' : ''; ?>>Pass</option>
                            <option value="fail" <?php echo (isset($_POST['quality_check']) && $_POST['quality_check'] == 'fail') ? 'selected' : ''; ?>>Fail</option>
                        </select>
                    </div>
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <textarea name="notes" id="notes" class="form-control" rows="4" 
                                  placeholder="Additional notes or observations"><?php echo htmlspecialchars($_POST['notes'] ?? ''); ?></textarea>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>Add Processing Log
                            </button>
                        </div>
                    </div>
                </div>
...
function generateBatchId() {
    ...
    const batchId = `BATCH-S${section}-${year}${month}${day}-${random}`;
