...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    $deleteId = $_POST['item_id'] ?? 0;
    
    if ($deleteId && is_numeric($deleteId) && SessionManager::hasPermission('inventory_manage')) {
        try {
            // Check if item exists and user has permission
            $checkStmt = $conn->prepare("SELECT * FROM inventory WHERE id = ?");
            $checkStmt->execute([$deleteId]);
            $itemToDelete = $checkStmt->fetch();
            
            if ($itemToDelete) {
                // Check section permissions for non-admin users
                if ($userInfo['role'] !== 'admin' && !in_array($itemToDelete['section'], $userInfo['sections'])) {
                    header('Location: index.php?error=access_denied');
                    exit();
                }
                
                // Check if item is used in any orders
                $orderCheckStmt = $conn->prepare("SELECT COUNT(*) as count FROM order_items WHERE inventory_id = ?");
                $orderCheckStmt->execute([$deleteId]);
                $orderCount = $orderCheckStmt->fetch()['count'];
                
                if ($orderCount > 0) {
                    header('Location: index.php?error=item_in_use');
                    exit();
                }
                ...
                if ($result) {
                    // Log the activity
                    logActivity('inventory_deleted', "Inventory item deleted: {$itemToDelete['item_name']} (ID: {$deleteId})", $userInfo['id']);
    ...
    <div class="alert alert-danger alert-dismissible fade show" role="alert">
        ...
        switch ($_GET['error']) {
            ...
            case 'item_not_found':
                ...
                break;
            case 'item_in_use':
                echo 'Cannot delete item: It is used in existing orders.';
                break;
            case 'delete_failed':
            ...
            case 'database_error':
                ...
                break;
            case 'invalid_item':
                echo 'Invalid inventory item specified.';
                break;
            default:
                                        ...
                                        <div class="btn-group" role="group">
                                            ...
                                            <?php if (SessionManager::hasPermission('inventory_manage')): ?>
                                                ...
                                                <button type="button" class="btn btn-sm btn-outline-danger" 
                                                        onclick="confirmDelete(<?php echo $item['id']; ?>, '<?php echo htmlspecialchars($item['item_name']); ?>')" 
                                                        title="Delete">
